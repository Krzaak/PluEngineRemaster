...
def write_or_update_init(path, generated_block):
    if os.path.exists(path):
        with open(path, "r", encoding="utf-8") as f:
            content = f.read()

        if AUTO_BEGIN in content and AUTO_END in content:
            # Podmień tylko region auto
            pattern = re.compile(
                re.escape(AUTO_BEGIN) + r".*?" + re.escape(AUTO_END),
                re.DOTALL
            )
            content = pattern.sub(generated_block, content)
        else:
            # Plik istnieje, ale bez regionu → dopinamy na końcu
            content += "\n\n" + generated_block
    else:
...
def generate_code(data):
    ...
    for proj_name, classes in project_groups.items():
        ...
        write_or_update_init(init_path, generated_block)


    print(f"Success! Generated Reflection data for {len(data)} classes in {OUTPUT_FILE}")
