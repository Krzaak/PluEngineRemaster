...
namespace Plu
{
    ...
    PLU_API BoneTransform BlendTransforms(const BoneTransform& a, const BoneTransform& b, float alpha);
    ...
    PLU_API BoneTransform BlendTransformsAdditive(const BoneTransform& base, const BoneTransform& additive, float alpha);

    // Pose-wide versions. outPose is resized to match and may alias a or b.
    // Mismatched sizes are clamped to the shorter input.
    PLU_API void BlendPoses(const Pose& a, const Pose& b, float alpha, Pose& outPose);
    PLU_API void BlendPosesAdditive(const Pose& base, const Pose& additive, float alpha, Pose& outPose);
    ...
    PLU_API void BlendPosesMasked(const Pose& a, const Pose& b, const DynamicArray<float>& boneWeights,
