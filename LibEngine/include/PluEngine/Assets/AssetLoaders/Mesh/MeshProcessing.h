...
namespace Plu::MeshProcessing
{
    ...
    PLU_API UInt32 PackNormal(const Vec3& normal);
    PLU_API UInt32 PackTangent(const Vec3& tangent, float sign);
    PLU_API UInt16 PackUV(float uv);
    PLU_API UInt32 PackColor(const aiColor4D& color);
